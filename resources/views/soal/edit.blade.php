...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('soal.update', request()->soal) }}" method="POST">
                        @method('PUT')
                        @csrf 
                        <h3>Soal</h3>
                        <div class="form-group row mb-0">
                            ...
                            <div class="col-sm-10">
                                ...
                                <input type="checkbox" id="is_aktif" value="1" name="is_aktif" {{ old('is_aktif', $soal->is_aktif ?? '') == '1' ? 'checked' : ''}} data-bootstrap-switch data-off-color="warning" data-on-color="success" data-on-text="Yes" data-off-text="No">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="teks" class="col-form-label">Teks</label>
                            <textarea class="form-control teks {{ $errors->has('teks') ? 'is-invalid' : '' }}" name="teks">{{ old('teks', $soal->teks ?? '') }}</textarea>
                        </div>
                        ...
                        <table class="table table-bordered table-striped table-hover" id="jawaban_tabel" style="width: 100%">
                            ...
                            <tbody>
                                @php $jawaban_teks = old('jawaban_teks', $soal->jawaban ?? []); @endphp
                                @foreach($jawaban_teks as $key => $item)
                                <tr> 
                                    <td> 
                                        <textarea class="form-control teks" id="jawaban_teks{{ $key }}" name="jawaban_teks[{{ $key }}]">{{ old('jawaban_teks.' . $key, $item->teks ?? '') }}</textarea>
                                    </td>
                                    <td> 
                                        <input type="hidden" name="is_jawaban_benar[{{ $key }}]" value="0">
                                        <input type="checkbox" value="1" name="is_jawaban_benar[{{ $key }}]" {{ old('is_jawaban_benar.' . $key,  $item->is_benar ?? '') == '1' ? 'checked' : ''}} data-bootstrap-switch data-off-color="warning" data-on-color="success" data-on-text="Yes" data-off-text="No">
                                    </td> 
                                </tr>
                                @endforeach
                            </tbody>
                        ...
                        <table class="table table-bordered table-striped table-hover" id="jawaban_tabel" style="width: 100%">
                            ...
                            <tbody>
                                @php $alasan_teks = old('alasan_teks', $soal->alasanJawaban ?? []); @endphp
                                @foreach($alasan_teks as $key => $item)
                                <tr> 
                                    <td> 
                                        <textarea class="form-control teks" id="alasan_teks{{ $key }}" name="alasan_teks[{{ $key }}]">{{ old('alasan_teks.' . $key, $item->teks ?? '') }}</textarea>
                                    </td>
                                    <td> 
                                        <input type="hidden" name="is_alasan_benar[{{ $key }}]" value="0">
                                        <input type="checkbox" value="1" name="is_alasan_benar[{{ $key }}]" {{ old('is_alasan_benar.' . $key,  $item->is_benar ?? '') == '1' ? 'checked' : ''}} data-bootstrap-switch data-off-color="warning" data-on-color="success" data-on-text="Yes" data-off-text="No">
                                    </td> 
                                </tr>
                                @endforeach
                            </tbody>
